...
st.set_page_config(page_title="Match View · Corners", page_icon="📅", layout="wide")
inject_css()
df = load_data()
...
<div style="padding:4px 0 16px 0;">
  <div style="display:flex;gap:10px;align-items:center;">
    ...
    st.markdown("#### Filters")
    teams_in_match = sorted(df[df["match"].astype(str)==focus]["team"].dropna().astype(str).unique().tolist())
...
if only_shots: f = f[f["is_shot"] == True]
...
teams_str = " vs ".join(teams_in_match[:2]) if len(teams_in_match) >= 2 else focus
...
c1, c2 = st.columns(2)
with c1:
    st.markdown("<div class='card'><div class='card-title'>Corners by Team</div>", unsafe_allow_html=True)
...
with c2:
    st.markdown("<div class='card'><div class='card-title'>Corner Timing</div><div class='card-sub'>When were corners taken?</div>", unsafe_allow_html=True)
    styled_histogram(_to_num(f["Minute_num"]), nbins=20, height=300)
    st.markdown("</div>", unsafe_allow_html=True)

c3, c4 = st.columns(2)
with c3:
    st.markdown("<div class='card'><div class='card-title'>Technique Distribution</div>", unsafe_allow_html=True)
    tech = f.groupby("technique", dropna=False).size().reset_index(name="n")
    styled_donut(tech, "technique", "n", height=280)
    st.markdown("</div>", unsafe_allow_html=True)

with c4:
    st.markdown("<div class='card'><div class='card-title'>Shot Outcomes</div>", unsafe_allow_html=True)
...
# Per-team sp_outcome
st.markdown("<div class='card'><div class='card-title'>Set Piece Outcomes by Team</div>", unsafe_allow_html=True)
...
# Event table
st.markdown("<div class='card'><div class='card-title'>All Corner Events</div>", unsafe_allow_html=True)
...
st.dataframe(f[cols].reset_index(drop=True), use_container_width=True, hide_index=True, height=340)
st.markdown("</div>", unsafe_allow_html=True)
